const wordGuessEvaluator = (req, res) => {
    ...
    for (let i = 0; i < correctAnswerArray.length; i++) {
      if (userInputArray[i] === correctAnswerArray[i]) {
        result.push({ letter: userInputArray[i], color: 'green', background: 'lightgreen', result: 'correct'});
        ...
        if (
          ...
          duplicateCounter++;
          result.push({ letter: userInputArray[i], color: 'red', background: 'lightpink', result: 'incorrect'});
        } else {
          result.push({ letter: userInputArray[i], color: 'yellow', background: 'lightgoldenrodyellow', result: 'misplaced'});
        }
      } else {
        result.push({ letter: userInputArray[i], color: 'red', background: 'lightpink', result: 'incorrect'});
      }
